...
from bot.keyboards.inline import language_keyboard, main_menu_keyboard, admin_menu_keyboard
from bot.services.database import async_session
from bot.services.settings import get_setting, refresh_cache
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user:
        shop_name = await get_setting("SHOP_NAME", settings.SHOP_NAME)
        if user.language == LANG_AMHARIC:
            text = f"✨ እንኳን ደህና መጡ {user.full_name}!\n\nወደ {shop_name} በደህና መጡ።"
        else:
            text = f"✨ Welcome back {user.full_name}!\n\nYou're at {shop_name}."
        await message.answer(text, reply_markup=main_menu_keyboard(user.language))
    else:
        await message.answer(
            "👋 Welcome to Crown Barber Shop!\n\n"
...
async def process_language(callback: CallbackQuery, state: FSMContext):
    ...
    if lang == LANG_AMHARIC:
        await callback.message.edit_text("🪪 ስምዎን ያስገቡ:")
    else:
...
async def process_name(message: Message, state: FSMContext):
    ...
    if len(full_name) < MIN_NAME_LENGTH or len(full_name) > MAX_NAME_LENGTH:
        ...
        if lang == LANG_AMHARIC:
            await message.answer(f"ከ{MIN_NAME_LENGTH} እስከ {MAX_NAME_LENGTH} ፊደላት ያስገቡ:")
        else:
    ...
    if lang == LANG_AMHARIC:
        await message.answer("📱 ስልክ ቁጥርዎን ያስገቡ (0911223344):")
    else:
        await message.answer("📱 Please enter your phone number (0911223344):")
...
async def process_phone(message: Message, state: FSMContext):
    ...
    if not validate_ethiopian_phone(phone):
        if lang == LANG_AMHARIC:
            await message.answer("ትክክለኛ ስልክ ቁጥር ያስገቡ (0911223344 ወይም +251911223344):")
        else:
    ...
    try:
        ...
        if lang == LANG_AMHARIC:
            await message.answer("የቴክኒክ ችግር ተከስቷል። እንደገና ይሞክሩ።")
        else:
            await message.answer("A technical error occurred. Please try again.")
        return

    if lang == LANG_AMHARIC:
        text = f"✅ ምዝገባ ተጠናቋል! እንኳን ደህና መጡ {full_name}!"
    else:
        text = f"✅ Registration complete! Welcome {full_name}!"
...
async def about_shop(callback: CallbackQuery):
    ...
    opening = await get_setting("OPENING_TIME", settings.OPENING_TIME)
